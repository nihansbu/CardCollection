...
- Die `ContentPanel`-Header-Bar ist mobil standardmaessig eingeklappt, damit Body und InfoPanel mehr nutzbare Flaeche bekommen. Oben mittig sitzt ein flacher CSS-Goldtab, der die Header-Bar ein- und ausfaehrt. Der Tab ist bewusst niedrig und breit, liegt eingeklappt am oberen Rand und ausgeklappt innerhalb der Topbar an ihrer unteren Kante; unter den Header-Stats bleibt genug Abstand, damit er keine Content-Elemente ueberdeckt.
...
- Deeds ist der erste Screen, der das neue Fantasy-Pergament-UI-Target testet: echtes generiertes Pergament-Board als Raster-Asset, rotes Ribbon-Ornament, warme Papierflaechen, dunkle gut lesbare Schrift, farbige Deed-Karten, weichere Progress-Strips und handgezeichnete Map-Elemente im unteren Body. Deeds startet bewusst mit sichtbarer Header-/Stats-Zone wie im Mockup, bleibt aber ueber den gemeinsamen Header-Toggle einklappbar. Die globale Shell, Bottom-Navigation und ContentPanel-Mechanik bleiben vorerst unveraendert.
